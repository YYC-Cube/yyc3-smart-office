...
function getPayloadConfigFromPayload(config: ChartConfig, _payload: RechartsPayload | unknown, key: string) {
  if (typeof _payload !== 'object' || _payload === null) {
    return undefined;
  }

  const typedPayload = _payload as RechartsPayload;
  const payloadPayload = typedPayload.payload;

  let configLabelKey: string = key;

  if (key in typedPayload && typeof (typedPayload as Record<string, unknown>)[key] === 'string') {
    configLabelKey = (typedPayload as Record<string, unknown>)[key] as string;
  } else if (
    payloadPayload &&
